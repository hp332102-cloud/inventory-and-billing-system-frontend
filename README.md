...
**Q1: What is the main purpose of this Inventory & Billing System?**

The system automates inventory management and billing processes while ensuring GST compliance. It reduces manual errors and provides business owners with real-time insights through comprehensive reporting and a user-friendly interface.

---

**Q2: How does the system handle GST calculations?**

GST is calculated automatically based on the customer's state. For intra-state transactions, CGST and SGST are applied equally (each at half the product's GST rate). For inter-state transactions, IGST is applied at the full GST rate. GST rates are configured per product and validated against standard Indian GST slabs.

---

**Q3: What security measures are implemented in the system?**

* **JWT Authentication** — All API routes are protected with token-based authentication.
* **Role-based Access Control** — Admins have full access; cashiers can only create and view invoices.
* **Bcrypt Password Hashing** — Passwords are never stored in plain text.
* **Input Validation** — All API inputs are validated server-side to prevent injection attacks.

---

**Q4: How does the discount system work?**

The system supports two levels of discounting:
* **Item-level discounts** — Applied per product line, either as a percentage of the item price or as a flat deduction. The discount reduces the taxable value before GST is computed.
* **Bill-level discounts** — Applied to the total taxable value after all item discounts, further reducing the overall tax liability.
Validation ensures discounts cannot exceed the product price or taxable value.

---

**Q5: What technologies were used in the frontend?**

The frontend is built with React.js for component architecture, Tailwind CSS for responsive styling, Chart.js for dashboard charts, React Router for client-side navigation, and Axios for API communication.

---

**Q6: How is data persistence handled?**

Data persistence is managed using MongoDB with Mongoose ODM. Each entity (products, customers, invoices, users) has a well-defined Mongoose schema with proper references and indexing for performance. All data is stored in a cloud-hosted MongoDB Atlas cluster.

---

**Q7: What is the role of the dashboard?**

The dashboard is the central overview screen. It displays live KPIs (total invoices, products, customers, total sales), a monthly sales trend chart, a GST breakdown by type, a list of recent invoices, and alerts for products that have fallen below their configured low-stock threshold.

---

**Q8: How are PDF invoices generated?**

PDF invoices are generated server-side using the PDFKit library. When a user requests a PDF, the backend assembles a professionally formatted document containing the company's details, customer information, an itemized billing table with GST columns, discount breakdowns, and the final total. The file is stored on the server and returned as a downloadable stream.

---

**Q9: What is the significance of HSN codes?**

HSN (Harmonized System of Nomenclature) codes are a standardized classification system used globally for identifying goods under GST. They are mandatory for GST compliance in India. Each product in the system can be assigned an HSN code, which appears on generated invoices and in GST reports for accurate tax categorization.

---

**Q10: How does the system handle stock management?**

Stock is updated automatically whenever an invoice is created — quantities are deducted in real time. Each product has a configurable `lowStockThreshold` field; when the current stock drops below this value, the product appears in the dashboard's low-stock alert panel. Administrators can manually adjust stock levels through the product management interface.
...
> *"Good morning, everyone. Today I'm excited to present our Inventory & Billing System — a comprehensive web application designed to streamline business operations with full GST compliance. This system automates inventory management and billing, and gives business owners real-time insights through an intuitive, modern interface."*

---

### Project Overview *(~1 minute)*
> *"Our system is built using modern web technologies: React.js on the frontend, Node.js and Express.js on the backend, and MongoDB for data storage. The core features include product management with configurable GST rates, a customer database with GSTIN validation, dynamic invoice generation with automatic GST calculations, and a comprehensive reporting dashboard."*

---

### Technical Deep Dive *(~1.5 minutes)*
> *"Let me walk you through the architecture. The frontend uses React components styled with Tailwind CSS and Chart.js for data visualization. The backend exposes RESTful APIs secured with JWT authentication and role-based access control — separating admin and cashier permissions. We use MongoDB with Mongoose for data persistence, and PDFKit for generating professional invoices. The system automatically determines GST type based on customer location: CGST/SGST for intra-state and IGST for inter-state transactions — fully compliant with Indian GST regulations."*

---

### Live Demo Highlights *(~1 minute)*
> *"In our demo, you'll see how easy it is to add products with HSN codes and GST rates, manage customers with GSTIN validation, and generate GST-compliant invoices with real-time calculations. We'll also walk through the analytics dashboard showing sales trends and the reporting module that produces downloadable PDF documents for official use."*

---

### Conclusion & Future Scope *(~30 seconds)*
> *"In conclusion, our system successfully addresses the challenges of manual billing and inventory management while maintaining GST compliance. The architecture is scalable and can be enhanced with features like barcode scanning, a mobile application, or integration with third-party accounting software. Thank you for your attention — I'm happy to take any questions."*
