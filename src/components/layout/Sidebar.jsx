...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside className={`fixed top-0 left-0 z-50 h-[100dvh] w-[260px] bg-[#222E43] text-slate-300 flex flex-col transition-transform duration-300 ease-in-out lg:translate-x-0 ${
            ...
            <nav className="space-y-1.5">
              {menuItems.map((item) => (
                <NavLink
                  ...
                  to={item.path}
                  onClick={() => setIsOpen(false)}
                  className={({ isActive }) => `flex items-center gap-4 px-4 py-[12px] rounded-lg text-[15px] font-medium transition-colors ${
                      isActive
            ...
            <nav className="space-y-1">
              {reportItems.map((item) => (
                <NavLink
                  ...
                  to={item.path}
                  onClick={() => setIsOpen(false)}
                  className={({ isActive }) => `flex items-center gap-4 px-4 py-[10px] rounded-lg text-[14px] font-medium transition-colors ${
                      isActive
